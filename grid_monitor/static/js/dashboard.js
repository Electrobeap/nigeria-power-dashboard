...
function visibleGenerationStats() {
    if (!chart) return seriesStats(generationReadings);
    const scale = chart.scales.x;
    if (!scale) return seriesStats(generationReadings);
    const from = Math.max(0, Math.ceil(scale.min));
    const to = Math.min(generationReadings.length - 1, Math.floor(scale.max));
    if (to < from) return seriesStats(generationReadings);
    return seriesStats(generationReadings.slice(from, to + 1));
}

function renderChartStats() {
    const stats = visibleGenerationStats();
    const set = (id, text) => { if (el[id]) el[id].textContent = text; };
    if (!stats) {
        ["stat-avg", "stat-max", "stat-min", "stat-sd", "stat-n"].forEach((id) => set(id, "..."));
        return;
    }
    set("stat-avg", formatMW(stats.mean));
    set("stat-max", formatMW(stats.max));
    set("stat-min", formatMW(stats.min));
    set("stat-sd", `±${formatMW(stats.sd)}`);
    set("stat-n", formatNumber(stats.count, 0));
}
...
function toggleBrush() {
    ...
    zoom.zoom.drag = brushEnabled
        ? { enabled: true, backgroundColor: "rgba(37, 99, 235, 0.15)", borderColor: "#2563EB", borderWidth: 1 }
        : { enabled: false };
    zoom.pan.enabled = !brushEnabled;
    if (el["chart-brush"]) el["chart-brush"].setAttribute("aria-pressed", brushEnabled ? "true" : "false");
    chart.update("none");
}
