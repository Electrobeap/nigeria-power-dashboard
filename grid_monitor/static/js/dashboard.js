...
// Backdrop, deliberately at the edge of perceptible: the bands say which
// operating region a reading sits in without competing with the series.
function generationBands() {
    return [
        { from: null, to: GENERATION_CRITICAL_MW, color: "rgba(220, 38, 38, 0.045)" },
        { from: GENERATION_CRITICAL_MW, to: GENERATION_WARNING_MW, color: "rgba(245, 158, 11, 0.04)" },
        { from: GENERATION_WARNING_MW, to: null, color: "rgba(0, 135, 81, 0.035)" },
    ];
}
...
// How much wall-clock time one plotted point should stand for. Drawing every
// stored reading at a wide window turns the raw series into a solid band of
// vertical strokes, which buries the trend it is supposed to give context to.
// These are display bins only: the stored readings, the statistics and the CSV
// export all still work from the full-resolution series.
const DISPLAY_BINS = [
    { upToHours: 24, minutes: 10 },
    { upToHours: 168, minutes: 60 },
    { upToHours: 720, minutes: 240 },
    { upToHours: 2160, minutes: 720 },
    { upToHours: Infinity, minutes: 1440 },
];

function displayBinMinutes(spanHours) {
    const band = DISPLAY_BINS.find((entry) => spanHours <= entry.upToHours);
    return (band || DISPLAY_BINS[DISPLAY_BINS.length - 1]).minutes;
}

// Wall-clock span of the window currently being plotted. Zooming shortens it,
// which selects a finer bin, so detail still increases as you go in.
function plotWindowSpanHours() {
    if (fullPoints.length < 2) return historyHours;
    const first = fullPoints[Math.max(0, plotWindow.from)];
    const last = fullPoints[Math.min(fullPoints.length - 1, plotWindow.to ?? fullPoints.length - 1)];
    const start = Date.parse(first?.timestamp);
    const end = Date.parse(last?.timestamp);
    if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start) return historyHours;
    return (end - start) / 3600000;
}

// The stricter of two limits: one point per display bin, and enough pixels
// between neighbouring points that they stay separable on screen.
function plotBudget() {
    const width = el.chart ? el.chart.getBoundingClientRect().width : 0;
    const usable = width > 0 ? width : (window.innerWidth || 1024);
    const byPixels = Math.round(usable / 4);
    const spanHours = plotWindowSpanHours();
    const byTime = Math.ceil((spanHours * 60) / displayBinMinutes(spanHours));
    return Math.max(40, Math.min(600, byPixels, byTime));
}
...
function outageMarkers(points, events) {
    ...
    // The plotted series is a display downsample, so an event rarely lands on
    // a drawn point exactly. Snap it to the nearest one instead of dropping it:
    // matching on an exact timestamp silently hid most events at wide windows.
    const times = points.map((point) => Date.parse(point?.timestamp));
    const known = times.filter((value) => !Number.isNaN(value));
    if (!known.length) return [];
    // Only events inside the plotted window get a marker; zooming still hides
    // the ones that have scrolled off rather than pinning them to the edge.
    const windowStart = Math.min(...known);
    const windowEnd = Math.max(...known);
    ...
    events.forEach((event) => {
        ...
        if (Number.isNaN(parsed) || parsed < windowStart || parsed > windowEnd) return;
        const index = nearestIndex(parsed);
        ...
        markers.push({
            ...
            // The glyph carries the signal; the tint only points at the column
            // underneath it, so it stays faint even when events cluster.
    ...
    if (!chart) {
        chart = new Chart(el.chart.getContext("2d"), {
            ...
            data: {
                ...
                datasets: [
                    {
                        // Background context, not the headline. Faint and hairline
                        // so it reads as the cloud of observations the trend is
                        // drawn through. tension 0 keeps it to straight segments
                        // between real readings - no invented curvature.
                        label: "Generation — observed",
                    ...
                    {
                        // The primary signal: drawn last, so it sits on top.
...
function selectHistoryRange(hours, button) {
    ...
    resetChartZoom();
    // resetChartZoom sizes the window against the series that is still loaded.
    // Leaving it there clamped the next, longer series to the old width - 7D
    // rendered only the oldest 24H of readings. null lets renderChart size the
    // window to whatever actually arrives.
    plotWindow = { from: 0, to: null };
